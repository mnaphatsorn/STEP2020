...
// Remove a free slot from the free list (remain the same as simple_remove_from_free_list)
...
void my_add_to_free_list(my_metadata_t* new_metadata) {
  // Add a free slot and sorted by address.
  assert(!new_metadata->next);
  my_metadata_t* metadata = my_heap.free_head;
  my_metadata_t* prev = NULL;
  while(metadata && metadata < new_metadata){
    prev = metadata;
    metadata = metadata->next;
  }
  prev->next = new_metadata;
  new_metadata->next = metadata;
  if (my_heap.free_head == metadata)
    my_heap.free_head = new_metadata;
  my_metadata_t* metadata_next = new_metadata->next;
  // Merge free slot
  void* ptr = new_metadata +1;
  void* ptr_prev = prev +1;

  //Merge with next free slot if connecting
  if (metadata_next){
    if (metadata_next == (my_metadata_t*)((char*)ptr + new_metadata->size)){
      if (metadata_next->next){
      new_metadata->next = metadata_next->next;
      metadata_next->next = NULL;
      }
      else{
        new_metadata->next = NULL;
    }
    new_metadata->size += metadata_next->size + sizeof(my_metadata_t);
    }
  }
  //Merge with previous free slot if connecting
  if (prev){
    if (new_metadata == (my_metadata_t*)((char*)ptr_prev + prev->size)){
      if (new_metadata->next){
      prev->next = new_metadata->next;
      new_metadata->next = NULL;
      }
      else{
        prev->next = NULL;
    }
    prev->size += new_metadata->size + sizeof(my_metadata_t);
    }
  }
}
...
void* my_malloc(size_t size) {
    ...
    // Find slot that has the similar size to wanted size
    while (metadata) {
        if(metadata-> size == size){
          best = metadata;
          best_prev = metadata_prev;
          break; //break the loop when exactly fit free slot is found
        }
    ...
    metadata->size = size;
    // Remove the free slot from the free list.
    my_remove_from_free_list(metadata, metadata_prev);
